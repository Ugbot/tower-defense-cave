...
/**
 * Manages gameplay and the game loop.
...
public class GameRunningState extends AbstractAppState {
    ...
    private boolean ghostAllowed = false;
    private long currentTime2;
    private long totalTime2;
    private Node teleporter;

    /**
     * Initializes gameplay. Called automatically when this state is attached.
     * @param stateManager
     * @param app 
     */
    @Override
    public void initialize(AppStateManager stateManager, Application app) {
        ...
        ambient.setColor(ColorRGBA.Gray.mult(1.8f));
        ...
        /* Spotlight to move along with camera. See update() */
        spot.setColor(ColorRGBA.Orange.mult(5f));
        spot.setSpotRange(1000);
        spot.setSpotOuterAngle(15 * FastMath.DEG_TO_RAD);
    ...
    private Geometry getWall(Vector3f location) {
        ...
        /* If a wall already exists, simply return its clone */
        if (simpleApp.getRootNode().getChild("wall") != null) {
            wall = (Geometry) simpleApp.getRootNode().getChild("wall").clone(true);
            wall.setLocalTranslation(location);
            wall.setLocalRotation(new Quaternion().fromAngleAxis(FastMath.DEG_TO_RAD * 180f, Vector3f.UNIT_Y));
            return wall;
        }
        
        /* Create wall */
        wall = (Geometry) simpleApp.getAssetManager().loadModel("Models/wall.j3o");
        wall.setName("wall");
    ...
    public Geometry getPlayerBase(Vector3f location) {
        Geometry base = (Geometry) simpleApp.getAssetManager().loadModel(
                "Models/base.j3o");
        Material mat = new Material(simpleApp.getAssetManager(),
                "Common/MatDefs/Light/Lighting.j3md");
        mat.setTexture("DiffuseMap", simpleApp.getAssetManager().loadTexture(
                "Textures/base/baseDiffuse.tga"));
        mat.setTexture("GlowMap", simpleApp.getAssetManager().loadTexture(
                "Textures/base/baseGlow.tga"));
     ...
     * @param location the location of where the geometry should be
     * @return the geometry representing one of the towers
     */
     ...
     * @param desiredNode the node you wish to have access to
     * @return the desired node (null if it doesn't exist)
     */
    ...
    public void update(float tpf) {
        ...
        if (gameOver) {
            JOptionPane.showMessageDialog(null, "Game Over"); // temporary; better way later
        ...
        if (isAddingTower) {
            ...
            spot.setDirection(simpleApp.getCamera().getDirection());
            spot.setPosition(simpleApp.getCamera().getLocation());
            return;
        }
        
        setAmbientColor(ColorRGBA.Gray);
        
        /* Update spot light's position and direction to match that of camera's */
        spot.setDirection(simpleApp.getCamera().getDirection());
        spot.setPosition(simpleApp.getCamera().getLocation());
        ...
        if (currentTime2 - totalTime2 >= 30000 && !ghostAllowed) {
            ghostAllowed = true;
            /* Test teleporter */
    ...
    /**
     * Sets the value of gameOver. For now only used in update() method to 
