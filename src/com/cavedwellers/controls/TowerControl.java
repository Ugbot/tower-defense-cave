...
public class TowerControl extends AbstractControl 
{
    ...
    public void controlUpdate(float tpf) 
    {
        ...
        {
            if (enemies.get(i).getLocalTranslation().getZ() - spatial.getLocalTranslation().getZ() < 10 
                || enemies.get(i).getLocalTranslation().getX() - spatial.getLocalTranslation().getX() < 10) 
            {
        ...
        if (reachable.size() > 0) 
        {
            ...
            {
                // FIXME: Ugly hack. First thing on the list to fix
    ...
    public void useBeam(EnemyControl enemy) 
    {
        ...
        beamNode.attachChild(getBeam(beamStart, enemy.getSpatial().getLocalTranslation()));
